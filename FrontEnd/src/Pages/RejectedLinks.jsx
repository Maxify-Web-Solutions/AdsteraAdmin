    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <h1 className="text-2xl sm:text-3xl font-bold text-white">Rejected Smartlinks</h1>
            </div>

            <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-700">
                    <h2 className="text-lg font-semibold text-white">Rejected Links List</h2>
                </div>
                            ...
                            {loading ? (
                                ...
                                links.map((item) => (
                                    <tr key={item._id} className="hover:bg-slate-700/30 transition">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="px-2 py-1 rounded text-xs font-medium capitalize bg-red-500/10 text-red-500 border border-red-500/20">
                                                {item.status}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                ...
                                <tr>
                                    <td colSpan="6" className="px-6 py-8 text-center">
                                        {error ? <span className="text-red-400">{typeof error === 'object' ? error.message || JSON.stringify(error) : error}</span> : "No rejected links found"}
                                    </td>
                                </tr>
                            )}
                        </tbody>
