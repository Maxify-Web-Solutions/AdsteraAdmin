    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-white tracking-tight">Approved Smartlinks</h1>
                    <p className="text-slate-400 mt-1">View active and approved tracking links.</p>
                </div>
                ...
                <div className="relative w-full md:w-72">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent placeholder-slate-500 transition-all"
                        value={searchTerm}
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-xl overflow-hidden">
                                    ...
                                    <tr key={item._id} className="group hover:bg-slate-700/40 transition duration-200">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-full bg-slate-700 flex items-center justify-center text-emerald-400 font-bold text-lg border border-slate-600">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <div className="flex items-center gap-2 text-xs text-slate-500">
                                                    <span className="font-mono bg-slate-900 px-1.5 py-0.5 rounded text-emerald-300">#{item.linkId}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                             <div className="flex flex-col gap-1 max-w-[220px]">
                                                ...
                                                {item.redirectUrl && (
                                                    <div className="flex items-center gap-1.5 text-xs text-slate-500 pl-1">
                                                        ...
                                                        <a href={item.redirectUrl} target="_blank" rel="noopener noreferrer" className="truncate hover:text-emerald-400 transition flex items-center gap-1">
                                                            {item.redirectUrl}
                                ...
                                <tr>
                                    <td colSpan="6" className="px-6 py-16 text-center text-slate-400">
                                        <div className="flex flex-col items-center justify-center gap-3">
                                            ...
                                            <p className="text-lg font-medium text-slate-300">No approved links found</p>
                                            <p className="text-sm">{searchTerm ? `No results match "${searchTerm}"` : "Active approved links will appear here."}</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
                ...
                <div className="px-6 py-4 border-t border-slate-700 bg-slate-800/50 flex items-center justify-between text-xs text-slate-400">
                    <span>Showing {filteredLinks.length} of {links.length} approved links</span>
                </div>
            </div>
        </div>
    )
